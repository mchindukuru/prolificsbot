...
namespace ProlificsBot.Helpers
{
    using Microsoft.Extensions.Options;
    using ProlificsBot.Models;
    using ProlificsBot.Models.Configuration;
    ...
    public class DevOpsHelper
    {
        ...
        public async Task<ReleaseManagementTask> MapToReleaseManagementTask (WorkItem workItem)
        {
            ...
            emailList = emailList.Append(GetEmailOrName(workItem.Resource.Fields[Constant.CreatedByKey], true));

            if (isAssignedToPresent)
            {
                emailList = emailList.Append(GetEmailOrName(assignedToString, true));
            }
            ...
            var releaseManagementTask = new ReleaseManagementTask
            {
                ...
                AssignedToProfileImage = isAssignedToPresent ? await graphHelper.GetProfilePictureByUserPrincipalNameAsync(GetEmailOrName(assignedToString, true)) : "",
                ...
                CreatedByProfileImage = await graphHelper.GetProfilePictureByUserPrincipalNameAsync(GetEmailOrName(workItem.Resource.Fields[Constant.CreatedByKey], true))
            };
